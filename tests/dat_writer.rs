...
mod test_dat_writer {
    ...
    fn writer_should_create_file_on_disk_if_missing() {
        ...
        // Execute
        let result = dat_package::write_package_to_path(package, tmp_path);
    ...
    fn entry_order_should_be_retained_between_writes() {
        ...
        let package = dat_package::read_from_path(TEST_DAT_PATH).unwrap();
        let order_before_write = package.inner_paths();

        // Execute
        let result = dat_package::write_package_to_path(package, tmp_path);
        assert!(result.is_ok());
        let package = dat_package::read_from_path(tmp_path).unwrap();
